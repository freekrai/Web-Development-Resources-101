...
add_filter("get_the_excerpt", "tfe_put_image");
	...
	if($_GET['page']=='thumbsexcerpts'){
		if('save'==$_REQUEST['action']){
			foreach($optionsTfE as $value){
				...
				//echo '<br>pune '.$value['id'].$_REQUEST[$value['id']];
			}
			header("Location: options-general.php?page=thumbsexcerpts&saved=true");
			die;
		}
	}
	//add_management_page('ThumbsExcerpts', 'Thumbs Excerpts', 'manage_options', 'thumbsexcerpts', 'rknthumb');
	add_options_page('ThumbsExcerpts', 'Thumbs Excerpts', 'administrator', 'thumbsexcerpts', 'rknthumb');
...
$optionsTfE=array(
	...
	array(	"name" => "Align",
			"desc" => '<strong>Align:</strong> You can choose between left and right align.',
			"id" => "tfe_align",
			"std" => "left",
	...
	array(	"name" => "Regenerate",
			"desc" => '<strong>Regenerate:</strong> Useful is for some reason, for a limited number of post, you have lost the generated thumbnails.<br>To regenerate all the thumbnails, use <strong>Regenerate Thumbnails</strong> plugin, signed by Viper007Bond.',
			"id" => "tfe_regenerate",
			"std" => "no",
			"options" => array('yes','no'),
			"type" => "select"
...
<p>What's new from the previous version? In 2.0 the plugin was rewritten to be XHTML valid, smarter and better. For example now it will work even with templates which do not use excerpts (will force the design to use excerpts). It has a setting page in WordPress back-end (which you see now) from where you can easily setup different aspects (previously you had to edit the PHP file). And yes, there is a default image for posts without images (but you can choose not to use it).

<h2>Questions you may ask</h2>

<p><b>I don't use WordPress 2.9. It will crash my blog?</b>
<br>No, but it's better to use latest WordPress. Should work from 2.5, but I've only tested on 2.8.
...
function tfe_put_image($excerpt){
	...
	$find=false;
	if(function_exists('has_post_image'))
		//you have WP 2.9 - do you use thumbnail feature, than I use it
		if(has_post_image($id)){
			$plus='<a href="'.get_permalink().'">'.get_the_post_image($id,array($width,150),array('class'=>'align'.$align.' tfe')).'</a>';
			$find=true;
		}
	if(!$find)
		//no thumbnail defined with 2.9 feature... than we go the old way
		$plus='<a href="'.get_permalink().'">'.tfe_get_image($id,$content,$align,$width,$height,$default,$regenerate,$default_src).'</a>';
	return $plus.$excerpt;
}
...
function tfe_change_to_excerpts($content){
	...
	$on_home=(get_settings('tfe_on_home')==''?'yes':get_settings('tfe_on_home'));
	$on_archives=(get_settings('tfe_on_archives')==''?'yes':get_settings('tfe_on_archives'));
	$on_search=(get_settings('tfe_on_search')==''?'yes':get_settings('tfe_on_search'));
